...
// Structure declaration for AppWidgets
typedef struct {
    GtkWidget *window;
    GtkWidget *stack;
    GtkWidget *login_box;
...
static void load_channel_history(AppWidgets *widgets, uint32_t channel_id);
...
static void update_chat_history(AppWidgets *widgets, const char *sender, const char *message, const char *channel_name) {
    ...
    // Sanitize all strings to ensure valid UTF-8
    const char *safe_sender = sender ? sanitize_utf8(sender) : "Unknown";
    const char *safe_message = sanitize_utf8(message);
    ...
    char formatted_msg[BUFFER_SIZE];
    snprintf(formatted_msg, sizeof(formatted_msg), "[%s] %s: %s\n", 
             time_str, safe_sender, safe_message);
...
static gboolean update_chat_history_from_network(gpointer user_data) {
    ...
    // Update chat history with the proper display name
    update_chat_history(data->widgets, display_name, data->content, "");
